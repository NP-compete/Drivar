...
public class TwilightCalculator {
    ...
    public void calculateTwilight(long time, double latiude, double longitude) {
        ...
        // true anomaly
        final float trueAnomaly = (float) (meanAnomaly + C1 * sin(meanAnomaly) + C2
                * sin(2 * meanAnomaly) + C3 * sin(3 * meanAnomaly));
        ...
        double solarTransitJ2000 = n + J0 + arcLongitude + 0.0053f * sin(meanAnomaly)
                + -0.0069f * sin(2 * solarLng);

        // declination of sun
        double solarDec = Math.asin(sin(solarLng) * sin(OBLIQUITY));

        final double latRad = latiude * DEGREES_TO_RADIANS;

        double cosHourAngle = (sin(ALTIDUTE_CORRECTION_CIVIL_TWILIGHT) - sin(latRad)
                * sin(solarDec)) / (Math.cos(latRad) * Math.cos(solarDec));
